This fork provides usb midi host function to the project author Copych code. Logs and gui are disabled. To change midi input mode and enable gui check config.h. For usb midi host function to work enable usb host on ESP32-S3 and in Arduino IDE change USB-Mode to USB-OTG(TinyUSB).
